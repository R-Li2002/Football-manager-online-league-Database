...
- 冠军杯、联盟杯和无铭剑杯在积分榜视图下统一显示“小组赛阶段 / 淘汰赛阶段”阶段轨道
- 淘汰赛阶段继续承载现有对称晋级图、比分编辑、晋级判定和一键初始化能力
- 小组赛阶段作为独立数据承载入口；当前数据模型尚未配置分组、组内赛程和小组积分时，页面明确显示“本届尚未录入”，不伪造小组数量、晋级名额或积分数据
- 淘汰赛初始化按钮仅在淘汰赛阶段显示，避免用户在小组赛视图误以为初始化会创建小组数据
- 当前阶段通过 `cupPhase=group|knockout` 写入 URL 和 SPA history，刷新、前进和后退会保留阶段选择
