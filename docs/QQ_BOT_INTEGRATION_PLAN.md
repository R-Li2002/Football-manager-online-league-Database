...
- 将帮助文本升级为按球员、球队、战力、比赛、新闻分组的交互式菜单。
- 基于 Waiter 扩展连续会话：名单和排行榜支持上一页、下一页、页码跳转，球员结果支持继续查看成长形态、工资和对比，并统一支持“取消”。
- 限流时返回明确的剩余等待时间，避免静默无响应。
- 增加常见自然语言命令和错别字容错，同时保留 Alconna 作为结构化命令入口。

### 17.2 主站能力接入

- 接入球员对比图，支持当前形态与 `+1` 至 `+5` 成长形态混合比较。
- 接入加权战力榜、HEIGO战力榜、潜力榜，并支持球队、位置和成长形态筛选。
- 提供机器人版高级球员搜索，覆盖年龄、位置、属性、战力和 `PA - CA` 潜力空间。
- 增加球队战力概览，包括平均战力、队内前三、潜力球员、工资结构与薄弱位置。

### 17.3 联赛助手

- 提供积分榜、本轮赛程、最近赛果、射手榜、助攻榜、伤停与停赛查询。
- 提供可配置的早报、赛程、赛果、伤停、名单更新和工作任务提醒，并允许查看及取消订阅。
- 为有权限的工作账号提供只读待办摘要和主站工作台跳转，不在群内开放数据写操作。

### 17.4 稳定性与维护

- 统一处理主站 API 超时、图片渲染失败和 QQ 发图失败，返回可读提示及短错误编号。
- 对球队目录、排行榜和重复图片请求增加短期缓存、请求去重和渲染并发限制。
- 记录匿名命令类型、成功率、耗时和无匹配关键词，用于决定后续功能优先级。
- 增加覆盖全部球队、中文别名、分页和图片渲染端点的自动化巡检。
