...
class FrontendStaticAssetVersionTests(unittest.TestCase):
    def test_root_page_appends_version_to_static_assets(self):
        ...
        self.assertIn(f'/static/js/app.home.js?v={STATIC_ASSET_VERSION}', response.text)
        self.assertIn(f'/static/app.js?v={STATIC_ASSET_VERSION}', response.text)
        self.assertNotIn(f'/static/js/app.players.js?v={STATIC_ASSET_VERSION}', response.text)
        self.assertNotIn(f'/static/js/app.competition.js?v={STATIC_ASSET_VERSION}', response.text)
        self.assertNotIn(f'/static/js/app.database.js?v={STATIC_ASSET_VERSION}', response.text)
